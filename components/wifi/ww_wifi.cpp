...
void wifi::initSta(wifi_mode_t wifi_mode, const char *ssid,
                   const char *password) {
  esp_err_t initError;
  ...
  initError = esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID,
                                                  wifiEventHandler, NULL, NULL);
  if (initError)
    ESP_LOGW(TAG, "event handler: %d", initError);
  // register TCP/IP related events
  initError = esp_event_handler_instance_register(IP_EVENT, ESP_EVENT_ANY_ID,
                                                  wifiEventHandler, NULL, NULL);
  if (initError)
...
wifi::State wifi::getState() { return currentState; }
