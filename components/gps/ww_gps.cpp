...
int parseGpsData(const uart_event_t *event) {
  ...
  if (recivedBytes > 0) {
    ...
    for (int i = 0; i < charCount - 1; i++) {
      ...
      if (size < 78 &&
          (size - 1) ==
              strlen(newLineCharPos[i])) // check if sentance is less then max
                                         // allowed nmea size 78 and '\0' is at
                                         // correct position
      {
        // ESP_LOGI(TAG, "size: %d and lenght: %d", size,
        // strlen(newLineCharPos[i])); ESP_LOGI(TAG, "%s", newLineCharPos[i]);
